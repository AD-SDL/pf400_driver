...
# pf400 Library
from pf400_client.publish_pf400_state import *
from pf400_client.publish_pf400_state import _update_pf400_state
from pf400_client.publish_pf400_state import heartbeat_transmitter
...
# TODO: figure out how to integrate pf400 code
'''
    This is the pf400TransferHandler class. The purpose of this class is to execute tasks running on the Manager's queue and alert the system of state changes and program status. 

    It is undecided if this package is directly running on the pf400 or if it will follow the same model as the OT2 
'''
class PF400TransferHandler(Node):
    def __init__(self, name):
        ...
        if status == self.status["ERROR"]:
            self.get_logger().fatal("Unable to get id from pf400 manager, exiting...")
        ...
        self.get_logger().info(
            "pf400 Transfer handler for ID: %s name: %s initialization completed"
            % (self.id, self.name)
        )
    ...
    '''
        ...
        No need to lock the pf400 this is done by the manager
    ...
    def get_next_transfer(self):
        ...
        get_next_transfer_cli = self.create_client(
            GetNextTransfer, "/pf400/%s/get_next_transfer" % self.id
        )
        ...
        try:
            ...
            output_msg = pf400_transfer(job, from_name, to_name) # Do an pf400 transfer TODO: error handling
        ...
        completed_transfer_pub = self.create_publisher(
            CompletedTransfer, "/pf400/%s/completed_transfer" % self.id, 10
        )
    ...
    def set_state(self, new_state):
        ...
        status = retry(self, _update_pf400_state, 1, 0, args) # if it fails it usually isn't something that will be fixed upon a retry
    ...
    def pf400_state_update_callback(self, msg):
        ...
        if(self.current_state == self.state['ERROR']):
            self.get_logger().error("Can't change state, the state of pf400 %s is already error"%msg.id)
    ...
    def run(self):
        ...
        while rclpy.ok():
            ...
            try: 
                ...
                if(status == self.status['ERROR']):
                    raise Exception("Unexpected Error occured in pf400TransferHandler get_next_transfer operation")
            except Exception as e: 
                self.get_logger().error("Error occured: %r" % (e,))
...
def main(args=None):
    ...
    pf400_transfer_node = PF400TransferHandler("Temp")
    try:
        spin_thread = Thread(target=pf400_transfer_node.run, args=())
        spin_thread.start()

        rclpy.spin(pf400_transfer_node)
    except Exception as e:
        pf400_transfer_node.get_logger().error("Error %r" % (e,))
    except:
        pf400_transfer_node.get_logger().fatal("Terminating...")

    # End
    pf400_transfer_node.dead = True # Force kill
    spin_thread.join()
    pf400_transfer_node.destroy_node()
    rclpy.shutdown()
