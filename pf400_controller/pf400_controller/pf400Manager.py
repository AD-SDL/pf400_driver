...
'''
    This class is the PF400Manager class. The purpose of the PF400Manager is to maintain the queue information for transfer requests, maintain state information, and provide a path to
    to the master's services. 
'''
class PF400Manager(Node):
    def __init__(self, name):
        ...
        # Node creation
        super().__init__("pf400_manager_" + name)  # User specifies name

        # Lock creation
        self.pf400_lock = Lock()  # Only one can access pf400 at a time
        ...
        self.get_id_service = self.create_service(
            GetId, "/pf400/%s/get_id" % self.name, self.get_id_handler
        )
        self.load_transfer_service = self.create_service(
            LoadTransfer, "/pf400/%s/load_transfer" % self.id, self.load_transfer_handler
        )
        self.get_next_transfer_service = self.create_service(
            GetNextTransfer,
            "/pf400/%s/get_next_transfer" % self.id,
            self.get_next_transfer_handler,
        )

        # Create subscribers
        self.pf400_state_update_sub = self.create_subscription(
            PF400StateUpdate,
            "/pf400/pf400_state_update",
            self.pf400_state_update_callback,
            10,
        )
        self.pf400_state_update_sub  # Prevent unused warning
        self.completed_transfer_sub = self.create_subscription(
            CompletedTransfer,
            "/pf400/%s/completed_transfer" % self.id,
    ...
    def get_next_transfer_handler(self, request, response):
        ...
        if(self.current_state == self.state['ERROR']):
            self.get_logger().error("pf400 in error state")
            ...
            self.get_logger().error("pf400 in busy state")
    ...
    def load_transfer_handler(self, request, response):  # TODO: error handling
        ...
        if completed == True:
            response.status = response.SUCCESS
            self.pf400_lock.release()  # Realise lock
            return response
    ...
    # Service to update the state of the pf400
    def pf400_state_update_callback(self, msg):
        ...
        # Bring to attention
        self.get_logger().warning("pf400 state for id %s is now: %s"%(msg.id, msg.state)) #TODO: maybe convert to text instead of num code
...
def main(args=None):
    ...
    pf400_manager_node = PF400Manager("Temp")
    try:
        rclpy.spin(pf400_manager_node)
    except:
        pf400_manager_node.get_logger().error("Terminating...")

    # End
    args = []
    args.append(pf400_manager_node)
    status = retry(
        pf400_manager_node, _deregister_node, 10, 1.5, args
    )  # TODO: handle status
    pf400_manager_node.destroy_node()
    rclpy.shutdown()
