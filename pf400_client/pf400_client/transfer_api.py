...
'''
 This is an API that will begin a transfer request from_node to_node on the designated pf400_name_or_id. 

 Note: item is currently unused as we don't have anything programmed for it yet so it is just ignored
'''
def load_transfer(
    self, block_name1, block_name2, item, pf400_name_or_id
    ...
    # Get pf400 info
    pf400_entry = get_node_info(self, pf400_name_or_id)
    ...
    if from_entry["type"] == "-1":  # Doesn't exist
        ...
        return self.status["ERROR"]
    if pf400_entry["type"] == "-1":  # Doesn't exist
        self.get_logger().error("node: %s doesn't exist with master" % pf400_name_or_id)
        return self.status["ERROR"]
    ...
    pf400_name = pf400_entry["name"]
    pf400_id = pf400_entry["id"]
    ...
    load_transfer_cli = self.create_client(
        LoadTransfer, "/pf400/%s/load_transfer" % pf400_id
    )
...
def _load_transfer(args):
    ...
    )  # self, from_name_or_id, to_name_or_id, item, pf400_id
